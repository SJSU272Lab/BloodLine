...
function get_all_cars(req, res, next, usersToSecurityContext)
{
    ...
    tracing.create('ENTER', 'GET blockchain/assets/samples', {});
    ...
    return Util.queryChaincode(securityContext, 'get_samples', [])
    .then(function(data) {
        let cars = JSON.parse(data.toString());
        console.log(cars);
        cars.forEach(function(car) {
            tracing.create('INFO', 'GET blockchain/assets/samples', JSON.stringify(car));
            res.write(JSON.stringify(car)+'&&');
        });
        tracing.create('EXIT', 'GET blockchain/assets/samples', {});
        res.end('');
    })
    .catch(function(err) {
        res.status(400);
        ...
        error.error = true;
        error.message = err;
        tracing.create('ERROR', 'GET blockchain/assets/samples', err);
