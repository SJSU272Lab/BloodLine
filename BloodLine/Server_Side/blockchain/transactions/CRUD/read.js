...
function evaluate_transactions(req, res)
{
        ...
        for(let i = 0; i < result.transactions.length; i++)
    {
            ...
            let bloodID = transaction.payload.match(/[A-Z]{2}[0-9]{7}/g);
            if(JSON.stringify(transaction).indexOf(user_id) === -1 && validV5cs.indexOf(bloodID) === -1)
        {
                result.transactions.splice(i, 1);
                i--;
            }
            else if (validV5cs.indexOf(bloodID) === -1)
        {
                validV5cs += '['+bloodID+']';
            }
        }
