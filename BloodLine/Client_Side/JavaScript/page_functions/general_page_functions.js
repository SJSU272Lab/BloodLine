...
function errCreate(bloodID)
{
    $('#failHd span').html('Creation Failed');
    $('#failTransfer').show();
    $('#failTxt').html('Failed to create car with bloodID: '+bloodID);
    $('#loader').hide();
    $('#loaderMessages').html('');
}
...
function errScrap(numErrs, numSucc)
{
    ...
    $('#failTransfer').show();
    $('#failTxt').html('Completed with some errors: <br /><br />'+numErrs+'  failed.<br />'+numSucc+'  were successful');
    $('#loader').hide();
    $('#loaderMessages').html('');
}

function getAddress(tester)
{
    let maxAddrLn = 0;
    let html = '';
