		...
		*/
		...
		if($('#selVhclsTbl tr').length >= 1 && $("#recipientInfo").html() != "")
		{
			...
			$("#selVhclsTbl tr").each(function() 
			{
				...
				var bloodID = $(this).find('.bloodID').val();
								
				$('#chooseConfHd').html('<span>Transaction Complete</span>');
				$('#confTxt').html('Transaction committed to the blockchain. <br /><br />'+sendDets+'<br /><br />'+recDets+': '+$('.delName').html()+' (Account '+$('.accAddr').html()+')<br /><br />Samples: '+$('#selVhclsTbl tr').length)
		...
		{
			$('#failTransfer').show();
			$('#failTxt').html('You have not selected any samples to transfer.');
		}
		else{
			$('#failTransfer').show();
		...
		*/
		...
		if($('#selVhclsTbl tr').length >= 1)
		{
			...
			$("#selVhclsTbl tr").each(function()
			{
				...
				var v5cAddr = $(this).find('.bloodID').val();
				if(index == $('#selVhclsTbl tr').length)
				{
					last = true;
				}
				$('#confTxt').html('Transaction committed to the blockchain. <br /><br />'+sendDets+'<br /><br />Samples: '+$('#selVhclsTbl tr').length)
				
				var data = {}
				data.bloodID = v5cAddr;
		...
		{
			$('#failTransfer').show();
			$('#failTxt').html('You have not selected any samples to scrap.');
		}
	})
