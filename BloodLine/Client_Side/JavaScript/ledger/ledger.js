		...
		success: function(d) {
			...
			for(var i = d.transactions.length-1; i >= 0 ; i--)
			{
				...
				if(obj.payload.indexOf("create_sample_log") == -1)
				{
					...
					var bloodID = 'undefined';
					var timestamp = 'undefined';
					var caller = 'undefined';
					var arguments = 'undfined';
					if(type != "undefined")
					{
						bloodID = obj.payload.match(/[A-Z]{2}[0-9]{7}/g);
						caller = obj.caller;
						...
						arguments = payload.substring(payload.indexOf(function_name)+function_name.length, payload.indexOf(bloodID)).trim();
						if(!found_cars.hasOwnProperty(bloodID))
						{
							found_cars[bloodID] = [];
						}
						found_cars[bloodID].push({"function_name": function_name, "args": arguments});
					}

					if(type == "Transfer")
					{
						var bin = get_update("bin", bloodID);
						var type = get_update("type", bloodID);
						var HIV = get_update("hiv", bloodID);
						var date = get_update("date", bloodID);
						var name = get_update("name", bloodID);
						...
						$('<tr class="retrievedRw" ><td class="smlBrk"></td><td style="width:1%; white-space:nowrap" class="transRw">['+bloodID+'] </td><td class="transRw" style="width:1%; white-space:nowrap"><span class="type" >Transfer</span><span class="message">: '+caller+' &rarr; '+arguments+'</span></td><td colspan="2" class="transRw">'+carDetails+'</td><td class="transRw txtRight">'+timestamp+'</td><td class="smlBrk"></td></tr>').insertAfter('#insAft')

					}
					if(type == "Create")
					{
						$('<tr class="retrievedRw" ><td class="smlBrk"></td><td style="width:1%; white-space:nowrap" class="transRw">['+bloodID+'] </td><td class="transRw" style="width:1%; white-space:nowrap"><span class="type" >Create</span><span class="message">: '+caller+'</span></td><td colspan="2" class="transRw">Create Sample</td><td class="transRw txtRight">'+timestamp+'</td><td class="smlBrk"></td></tr>').insertAfter('#insAft')
					}
					if(type == "Update")
					{
						var prev = get_update(update_type.toLowerCase(), bloodID);
						$('<tr class="retrievedRw " ><td class="smlBrk"></td><td style="width:1%; white-space:nowrap" class="transRw">['+bloodID+'] </td><td class="transRw" style="width:1%; white-space:nowrap"><span class="type" >Update</span><span class="message">: '+caller+'</span></td><td colspan="2" class="transRw">'+update_type+': '+prev+' &rarr; '+arguments+'</td><td class="transRw txtRight">'+timestamp+'</td><td class="smlBrk"></td></tr>').insertAfter('#insAft')
					}
					if(type == "Scrap")
					{
						$('<tr class="retrievedRw" ><td class="smlBrk"></td><td style="width:1%; white-space:nowrap" class="transRw">['+bloodID+'] </td><td class="transRw" style="width:1%; white-space:nowrap"><span class="type" >Scrap</span><span class="message">: '+caller+'</span></td><td colspan="2" class="transRw">Scrap Sample</td><td class="transRw txtRight">'+timestamp+'</td><td class="smlBrk"></td></tr>').insertAfter('#insAft')
					}
					if(failed)
					{
...
function get_update(field, bloodID)
{
	for(var i = found_cars[bloodID].length-2; i > -1; i--)
	{
		if(found_cars[bloodID][i].function_name == 'update_'+field)
		{
			return found_cars[bloodID][i].args;
		}
	}
	return 'undefined'
}
...
function sortbloodID(type)
{
	var arr = sortbloodIDIntoArray()
	if(type == 'desc')
	{
		arr = arr.reverse();
	}
