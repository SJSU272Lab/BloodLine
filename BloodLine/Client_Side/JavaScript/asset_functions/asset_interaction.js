...
function createAsset()
{
    ...
    $('#loaderMessages').html('<u>Creating Sample</u><br /><br /><span id="messages"></i>waiting...</i></span><br /></div>&nbsp;');

    let data = {};
    let found = [];
    let xhr = new XMLHttpRequest();
    xhr.open('POST', '/blockchain/assets/samples', true);
    ...
    xhr.onreadystatechange = function (){
        if(xhr.readyState === 4)
		{
            ...
            if(typeof JSON.parse(array[array.length -1]).error != 'undefined')
			{
                ...
                }(JSON.parse(array[array.length -1]).bloodID);
            }
            else
			{
                $('#loader img').hide();
                ...
                $('#confTxt').html('Created Sample: '+ JSON.parse(array[array.length - 1]).bloodID);
            }
        }
    };
...
function transferAsset()
{
    let found = [];
    let xhr = new XMLHttpRequest();
    xhr.open('PUT', '/blockchain/assets/samples/'+transferArray[transferIndex-1].bloodID+'/owner', true); //PUT (Updates)
    xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
    xhr.overrideMimeType('text/plain');
    xhr.onprogress = function () {
        let data = xhr.responseText;
...
function scrapAsset()
{
	...
	*/
    ...
    let objects = [];
    let xhr = new XMLHttpRequest();
    xhr.open('DELETE', 'blockchain/assets/samples/'+scrapArray[scrapIndex-1].bloodID, true); //As it refers to an v5, the path is: blockchain/assets/samples/vehicle/..
...
let bigData;
let increment;
function updateAsset(bin, type, hiv, name, date, bloodID, el)
{
	/*
	...
	*/
    ...
    bigData = [{'value':bin,'field':'BIN', 'title': 'BIN','bloodID':bloodID},{'value':type,'field':'type', 'title': 'Type','bloodID':bloodID},{'value':hiv,'field':'hiv', 'title': 'Hiv','bloodID':bloodID},{'value':name,'field':'name', 'title': 'Name','bloodID':bloodID},{'value':date,'field':'date', 'title': 'Date','bloodID':bloodID}];
...
function updateField()
{
    ...
    let xhr = new XMLHttpRequest();
    xhr.open('PUT', '/blockchain/assets/samples/'+bigData[increment].bloodID+'/'+bigData[increment].field, true);
    xhr.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
    xhr.overrideMimeType('text/plain');
    xhr.onprogress = function () {
        let data = xhr.responseText;
